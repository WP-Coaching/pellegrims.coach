...
This repository uses Next.js 16.1.7. Apply the current Next.js 16 conventions when they differ from the generic examples below:

- Treat `params`, `searchParams`, `cookies()`, and `headers()` as async APIs.
- Use `proxy.ts` for new request interception; do not add new `middleware.ts` patterns.
- Prefer the repository's existing `src/app/(site)` and `src/app/(payload)` route groups.
- Use the existing Payload cache-tag helpers in `src/lib/` when invalidating CMS-backed content.

Read `references/details.md` for the full pattern examples, then verify behavior against the installed Next.js version.
