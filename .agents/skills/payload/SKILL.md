...
This repository uses Payload 3.86.0 with `@payloadcms/db-sqlite`, npm, and the project scripts in `package.json`. Prefer the configured SQLite adapter and existing `src/payload.config.ts` over the MongoDB defaults in generic examples. Schema changes must follow the migration and type-generation rules in `AGENTS.md`.
