...
This repository currently uses Tailwind CSS 3.4.x with `tailwind.config.cjs`, PostCSS, and `@tailwind base/components/utilities` in `src/globals.css`. Treat the v4-specific `@theme`, `@custom-variant`, and `@import "tailwindcss"` examples below as migration guidance only. For ordinary UI work, preserve the existing v3 setup and use the project's `cn()` helper rather than introducing new component-library dependencies without an explicit request.
