...
class LanguageSwitchingHelper {
  ...
  async switchToLanguage(locale: string): Promise<void> {
    ...
    if (await this.isMobile()) {
      ...
      const languageLink = this.page
        .locator(`div.fixed.inset-0.z-50`)
        .getByRole("link", { name: switcherText });
  ...
  async verifyNavigation(language: "en" | "nl"): Promise<void> {
    const timeout = 10000;
    const navItems =
      language === "en" ? ["About", "Projects"] : ["Info", "Projecten"];
    ...
    if (await this.isMobile()) {
      ...
      for (const item of navItems) {
        ...
        await expect(
          this.page
            .locator(`div.fixed.inset-0.z-50 button:has-text("${item}")`)
            .first()
        ).toBeVisible({ timeout });
      }
      await this.closeMobileMenu();
      ...
      for (const item of navItems) {
        ...
        await expect(
          this.page
            .locator(`nav.hidden.lg\\:flex button:has-text("${item}")`)
            .first()
        ).toBeVisible({ timeout });
      }
    }
...
test.describe("Language Switching", () => {
  test("should switch between English and Dutch languages", async ({
    page,
  }) => {
  ...
  test("should display different content for different languages", async ({
    page,
  }) => {
    const helper = new LanguageSwitchingHelper(page);
